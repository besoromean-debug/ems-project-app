...
// Palitan ang 'COM3' kung anong port ang Arduino mo
const portSerial = new SerialPort({ path: 'COM3', baudRate: 9600 });
const parser = portSerial.pipe(new ReadlineParser({ delimiter: '\r\n' }));

let insertedCoins = 0;
parser.on('data', (data) => {
    // Kunwari nagse-send ang Arduino ng "1" bawat piso
    insertedCoins += parseInt(data);
    console.log("Current Balance:", insertedCoins);
});
