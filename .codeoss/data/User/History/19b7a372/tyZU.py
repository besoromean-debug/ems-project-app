...
# Kunin ang bucket name mula sa environment variable. 
# Siguraduhin na i-set ito gamit ang: export GCS_BUCKET_NAME="pangalan-ng-iyong-bucket"
...
def handle_upload():
    ...
    upload_status["filename"] = file.filename
        # 2. I-upload sa Google Cloud Storage gamit ang utility function
        gcs_uri = f"gs://{BUCKET_NAME}/uploads/{filename}"
        storage_utils.upload_file_to_gcs(local_path, gcs_uri)

        # 3. I-update ang status para sa polling ng Kiosk
        upload_status["uploaded"] = True
