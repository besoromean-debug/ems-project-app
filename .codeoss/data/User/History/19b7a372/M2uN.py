...
def handle_upload(session_id):
    ...
    if not bucket_name or bucket_name == "iyong-bucket-name":
        return "Server Error: GCS_BUCKET_NAME not set properly", 500

    try:
        # 1. I-save muna sa local temporary folder
        file.save(local_path)
        # 2. I-upload sa Google Cloud Storage gamit ang utility
        # 2. I-upload sa Google Cloud Storage gamit ang storage utility
        ...
        # 3. I-update ang status para sa polling ng Kiosk gamit ang session_id
        active_uploads[session_id] = {"uploaded": True, "filename": filename}
        return "Success! You can now look at the computer screen.", 200
