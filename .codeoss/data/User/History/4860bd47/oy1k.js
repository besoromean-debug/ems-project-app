...
const loginLimiter = rateLimit({
    ...
    max: 3, 
    handler: (req, res) => {
        res.redirect('/login?error=' + encodeURIComponent("Masyadong maraming mali. Ang iyong login access ay naka-lock muna ng 15 minuto."));
    }
});
...
app.post('/login', loginLimiter, async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            ...
            return res.redirect('/login?error=' + encodeURIComponent("Maling username o password."));
        }
        ...
        if (lockoutUntil && lockoutUntil > now) {
            ...
            return res.redirect('/login?error=' + encodeURIComponent(`Account locked. Subukan muli sa loob ng ${remaining} minuto.`));
        }
        ...
        if (match) {
            ...
            if (attempts >= 3) {
                ...
                return res.redirect('/login?error=' + encodeURIComponent("Account locked. Wala ka nang natitirang attempts. Balik ka ulit after 15 mins."));
            } else {
                await db.run( // Gamit ang db.run para sa SQLite
                    'UPDATE admin_users SET failed_attempts = ? WHERE id = ?',
                    [attempts, user.id]
                );
                return res.redirect('/login?error=' + encodeURIComponent(`Maling credentials. Mayroon ka na lang ${3 - attempts}/3 na subok.`));
            }
        }
    } catch (error) {
