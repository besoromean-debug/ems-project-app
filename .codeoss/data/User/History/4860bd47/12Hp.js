...
const loginLimiter = rateLimit({
    windowMs: 15 * 60 * 1000,
    max: 3, // Binago sa 3 attempts para sa buong login page per IP
    message: "Masyadong maraming mali. Ang iyong login access ay naka-lock muna ng 15 minuto."
});
...
app.post('/login', loginLimiter, async (req, res) => {
    ...
    try {
        ...
        if (!user) return res.status(401).send("Maling credentials.");
