    ...
    try {
        ...
        if (match) {
            ...
            if (attempts >= 3) {
                ...
                return res.redirect('/login?error=' + encodeURIComponent("Account locked due to 3 failed attempts. Please try again after 15 minutes."));
            } else {
                await db.run( // Gamit ang db.run para sa SQLite
                    'UPDATE admin_users SET failed_attempts = ? WHERE id = ?',
                    [attempts, user.id]
                );
                return res.redirect('/login?error=' + encodeURIComponent(`Invalid credentials. Attempts remaining: ${3 - attempts}/3`));
            }
        }
    } catch (error) {
