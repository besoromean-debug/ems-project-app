...
app.post('/api/employees', isAuthenticated, async (req, res) => {
    try {
        ...
        let { name, position, department } = req.body;
        
        // Siguraduhing walang extra spaces at hindi empty
...
app.post('/api/change-password', isAuthenticated, async (req, res) => {
    try {
        ...
        if (!match) {
            return res.status(400).json({ error: 'Maling kasalukuyang password.' });
        }

        const newHash = await bcrypt.hash(newPassword, 10);
        await db.run('UPDATE admin_users SET password_hash = ? WHERE username = ?', [newHash, username]);
        res.json({ message: 'Success! Napalitan na ang iyong password.' });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
