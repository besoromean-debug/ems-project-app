...
app.post('/login', loginLimiter, async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            ...
            return res.redirect('/login?error=' + encodeURIComponent("Invalid username or password. No account found."));
        }
        ...
        if (lockoutUntil && lockoutUntil > now) {
            ...
            return res.redirect('/login?error=' + encodeURIComponent(`Account is locked. Please try again in ${remaining} minute(s).`));
        }
        ...
        if (match) {
            ...
            console.log(`[Security] Failed attempt ${attempts}/3 for user: ${username}`);
            ...
            if (attempts >= 3) {
                ...
                return res.redirect('/login?error=' + encodeURIComponent("Account locked due to 3 failed attempts. Please try again in 15 minutes."));
            } else {
                ...
                return res.redirect('/login?error=' + encodeURIComponent(`Invalid password. ${remaining} attempt(s) remaining before lockout.`));
            }
        }
    } catch (error) {
